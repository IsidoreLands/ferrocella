            ...
            if SERIAL_AVAILABLE:
                try:
                    for side in self.sides:
                        self.sides[side]["ser"] = serial.Serial(
                            self.sides[side]["port"], 9600, timeout=1
                        )
    ...
    def _capture_baselines(self):
        ...
        if self.camera:
            try:
                ...
                self.visual_grid = (
                    cv2.cvtColor(self.raw_capture.array, cv2.COLOR_BGR2GRAY) / 255.0
                )
    ...
    def _read_arduino(self, side):
        if self.mock_mode or not self.sides[side].get("ser"):
            ...
            return {
                "resistance": 1e-9,
                "capacitance": 0.0,
                "permeability": 0.5 + np.sin(t * 0.1) * 0.5,
                "magnetism": random.uniform(0, 0.2),
                "permittivity": 1.0,
                "dielectricity": 0.0,
                "laser": random.uniform(0, 1),
            }
